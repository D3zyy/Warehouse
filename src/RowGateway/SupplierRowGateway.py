...
class SupplierRowGateway(RowGateway):
    ...
    def create(self):
        ...
        #Create new address
                        ...
                        phone_number = validate_phone_number()
                        query = "INSERT INTO  Suppliers(name,address_id,contact_number) VALUES(%s,%s,%s)"
                        self.database_connector.execute_query_with_commit(query, (name_of_supplier,addres[0][0],phone_number)) 
                        print("Uspesne jste vytvorili noveho dodavatele! \n")
    def update(self):
        while True:
                #validate id
    ...
    def delete(self):
        while True:
                ...
                if id_supplier == True:
                    ...
                    confirmation = input("Jste si jisty ze chcete smazat dodavatele  a i vsechny jeho dodavky od nej pro potvrzeni napiste: smazat \n Pro storno napise : storno\n")
                    match confirmation:
                          case "smazat":
                                #First delete parent rows
                                query = "DELETE FROM Purchases WHERE supplier_id = %s"
